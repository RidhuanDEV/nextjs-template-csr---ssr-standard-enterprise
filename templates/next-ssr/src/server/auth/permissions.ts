...
export function hasAnyPermission(session: JwtPayload, permissions: string[]): boolean {
  return permissions.some((p) => session.permissions.includes(p));
}

export function hasAllPermissions(session: JwtPayload, permissions: string[]): boolean {
...
export function requirePermission(session: JwtPayload, permission: string): void {

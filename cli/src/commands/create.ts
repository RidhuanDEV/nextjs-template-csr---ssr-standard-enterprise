...
function resolveTemplatesRoot(): string {
  ...
  // Walk upward from src/commands/ to find the monorepo root (parent of cli/)
  let dir = thisDir;
  for (let i = 0; i < 10; i++) {
    ...
    .action(async (name: string, options: { template?: string }) => {
        ...
        const { template } = await inquirer.prompt<{ template: TemplateType }>([
          {
            ...
            choices: [
              {
                name: "CSR - Client-Side Rendering (frontend only)",
                value: "next-csr",
              },
              {
                name: "SSR - Server-Side Rendering (fullstack with Prisma, Redis, Auth)",
                value: "next-ssr",
              },
            ],
          },
      ...
      await fs.copy(templateDir, targetDir, {
        filter: (src: string) => {
          ...
          return !["node_modules", ".next", ".git", "dist"].includes(basename);
        },
      });
